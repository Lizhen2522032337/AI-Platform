...
```text
React -> Nginx -> NestJS -> RabbitMQ -> Worker -> FastAPI -> 自建 RAG
                                                      |
                                               DeepSeek / 通义千问
                  |                     |          |
...
| Nginx | 唯一入口与反向代理 | `:80` |
| NestJS | 登录、RBAC、用户管理、知识文档管理、会话与任务 | `/api/` |
| Gin | JWT 与任务归属校验、SSE 实时状态 | `/realtime/` |
| FastAPI | 文档解析/向量化、ACL 检索、调用 DeepSeek/千问、统一流式输出 | 仅 Docker 内网 |
| Worker | 消费 RabbitMQ 并协调 AI 处理 | 仅 Docker 内网 |
| PostgreSQL | 持久化任务、用户和知识文档台账 | 仅 Docker 内网 |
| Redis | 实时状态缓存 | 仅 Docker 内网 |
| RabbitMQ | 持久化异步任务队列 | 仅 Docker 内网 |
| Qdrant | 向量存储 | 仅 Docker 内网 |
| MinIO | AI 结果文件与知识库原文件 | 仅 Docker 内网 |
...
- [自建 RAG 知识库与热配置](docs/knowledge-base.md)
- [首次手动部署与更新脚本](docs/deployment-scripts.md)
- [企业级构建、发布与回滚](docs/enterprise-deployment.md)

部署约束：Windows 家庭版不运行 Docker；所有镜像构建和容器运行均位于 Linux 虚拟机 `192.168.86.133`。真实配置固定保存在仓库外的 `/etc/enterprise-ai-platform`，大模型与 Embedding Key 保存在 `llm.env`；知识库检索参数保存在可热加载的 `knowledge-base.json`。
